...
export class WhatsAppService {
  ...
  constructor(accessToken?: string) {
    this.accessToken = accessToken || process.env.REACT_APP_WHATSAPP_ACCESS_TOKEN || ''
    this.phoneNumberId = process.env.REACT_APP_WHATSAPP_PHONE_NUMBER_ID || '158282837372377'
    this.businessAccountId = process.env.REACT_APP_WHATSAPP_BUSINESS_ACCOUNT_ID || '164297206767745'
    this.baseUrl = `https://graph.facebook.com/${process.env.REACT_APP_WHATSAPP_API_VERSION || 'v22.0'}`
    
    if (!this.accessToken) {
      console.warn('WhatsApp access token not provided. Please set REACT_APP_WHATSAPP_ACCESS_TOKEN in your .env file.')
    }
  }
