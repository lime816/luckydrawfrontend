...
export default function FlowPreviewPane({ flowId, flowName, onClose }: FlowPreviewPaneProps) {
    ...
    return (
      <div className="bg-white rounded-xl p-6 border-2 border-gray-200 shadow-sm">
        <div className="mb-6 pb-4 border-b border-gray-100">
          <h4 className="text-xl font-bold text-gray-900 mb-2">{typeof screen.title === 'string' ? screen.title : 'Untitled Screen'}</h4>
          <p className="text-sm text-gray-500 font-mono">ID: {typeof screen.id === 'string' ? screen.id : JSON.stringify(screen.id)}</p>
          {screen.terminal && (
            <span className="inline-block mt-3 px-3 py-1.5 bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 text-xs font-semibold rounded-full shadow-sm">
              ✓ Terminal Screen
            </span>
          )}
        </div>
        ...
        {screen.layout && (
          <div className="space-y-3">
            <div className="mb-4 px-3 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-100">
              <h5 className="font-semibold text-blue-900 text-sm">📐 Layout: {typeof screen.layout.type === 'string' ? screen.layout.type : 'Unknown'}</h5>
            </div>
                    ...
                    if (child.type === 'Form' && child.children && Array.isArray(child.children)) {
                      return (
                        <div key={idx} className="bg-white rounded-xl border-2 border-green-200 shadow-sm overflow-hidden">
                          <div className="flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-200">
                            <span className="text-sm font-semibold text-green-700">📝 Form Component</span>
                          </div>
                          <div className="p-6 space-y-6">
                            {child.children.map((formChild: any, formIdx: number) => (
                              <div key={`${idx}-${formIdx}`} className="space-y-2">
                                {renderFormElement(formChild)}
                                {formIdx < child.children.length - 1 && (
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-green-50 via-emerald-50 to-teal-50">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center shadow-lg">
              <Eye className="w-6 h-6 text-white" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-gray-900">Flow Preview</h2>
              <p className="text-sm text-gray-700 font-medium mt-1">{flowName}</p>
              <p className="text-xs text-gray-500 mt-0.5 font-mono">ID: {flowId}</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full">
              ...
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all shadow-md hover:shadow-lg"
              >
                Retry
              </button>
            </div>
                  ...
                  <div className="space-y-3 w-full max-w-md">
                    ...
                    {flowAsset._flowInfo?.preview?.download_url && (
                      <button
                        ...
                        className="w-full px-4 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white rounded-lg font-medium transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2"
                      >
                        <ExternalLink className="w-4 h-4" />
                        Download Flow JSON
                      </button>
